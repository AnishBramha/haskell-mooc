...
countNothings :: Num a => [Maybe a] -> a
countNothings xs = foldr countHelper 0 xs

countHelper :: Num a => Maybe a -> a -> a
-- countHelper Nothing acc = 1 + acc
-- countHelper (Just _) acc = acc

-- alternate
countHelper x acc = acc + maybe 1 (\_ -> 0) x
...
slHelper x acc = (fst acc + x, snd acc + 1)
...
concatHelper :: [a] -> [a] -> [a]
concatHelper ys acc = foldr (:) acc ys
