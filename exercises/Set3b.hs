...
sums i = sums' i 1
    where sums' n x
            | n < x = []
            | otherwise = (x * (x + 1)) `div` 2 : sums' n (x + 1)
...
indexDefault xs i def = if i < 0 then def else indexDefault' xs i def 0
    
    where indexDefault' [] n def j = def
          indexDefault' (x : xs) n def j
            | n == j = x
            | n > j = indexDefault' xs n def (j + 1)
...
sumsOf xs = sumsOf' xs 0
    ...
    where sumsOf' [] _ = []
          sumsOf' (x : xs) sum = sum' : sumsOf' xs sum'
