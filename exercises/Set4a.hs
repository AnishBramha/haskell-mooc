...
-- alternate:
allEqual [] = True
allEqual (x : xs) = foldr (\y acc -> (x == y) && acc) True xs
...
middle x y z
    | max y z >= x && x >= min y z = x
    | max x z >= y && y >= min x z = y
    | otherwise = z
...
rangeOf xs = abs $ maxi - mini

    where (maxi, mini) = foldr findMinMax (head xs, head xs) xs

          findMinMax :: Ord a => a -> (a, a) -> (a, a)
          findMinMax y (min', max') = (min min' y, max max' y)
...
longest :: Ord a => [[a]] -> [a]
longest xss = head $ sortBy compareLists xss
            
    where compareLists :: (Ord a) => [a] -> [a] -> Ordering
          compareLists xs ys = if lenCompare /= EQ then lenCompare

                               else compare (head xs) (head ys)

            where lenCompare = compare (length ys) (length xs)

-- alternate
-- longest = last . sortBy (comparing length) . reverse . sortBy (comaparing head)
...
incrementKey key xs = map (\(k,v) -> if k == key then (k, v + 1) else (k,v)) xs
...
average xs = (foldr (+) 0.0 xs) / (fromIntegral $ length xs)
...
winner scores player1 player2 = if score1 >= score2 then player1 else player2
...
freqs xs = freqs' xs Map.empty
    ...
    where freqs' [] freq = freq
          freqs' (x : xs) freq = 
            let freq' = case Map.lookup x freq of
                            Nothing -> Map.insert x 1 freq
                            Just f -> Map.insert x (f + 1) freq
            in freqs' xs freq'
...
transfer from to amount bank = if amount < 0 then bank

    else case Map.lookup from bank of
          Nothing -> bank
          Just f -> case Map.lookup to bank of
                        Nothing -> bank
                        Just t -> if diff < 0 then bank else Map.insert from (f - amount) $ Map.insert to (t + amount) bank
...
maxIndex arr = foldl1' findMax (indices arr)
    where findMax i j = if arr ! i > arr ! j then i else j
